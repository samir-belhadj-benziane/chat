...
class User extends DataBase
{
    ...
    public function register($username, $mail, $password, $date)
    {
        if (isset($username) and isset($mail) and isset($password)) {
            ...
            if ($usernamelenght >= 6 && $usernamelenght <= 18) {
                ...
                $message1 = "Le Nom D'utilisateur doit contenir 6 a 18 caractères ! ❌/1";
                $usernameaccess = 0;
            }
            ...
            if ($passwordlenght >= 4 && $passwordlenght <= 18) {
                ...
                $message4 = "Le Mot de Passe doit contenir 4 a 18 caractères ! ❌/1";
                $passwordaccess = 0;
            }

            if ($usernameaccess == 1 && $mailaccess == 1 && $passwordaccess == 1) {
                $passwordright = sha1($password);
                $register = $this->connect()->prepare("INSERT INTO users (username, mail, password, born) VALUES (?, ?, ?, ?)");
                $register->execute(array($username, $mail, $passwordright, $date));
    ...
    public function connection($usermail, $password)
    {
        ...
        if (!empty($usermail) and !empty($password) and $password != 'da39a3ee5e6b4b0d3255bfef95601890afd80709') {
            $getmail = $this->connect()->prepare("SELECT * FROM users WHERE mail = ? || phone = ?");
            $getmail->execute(array($usermail, $usermail));
            ...
            if ($mailaccess == 1) {
                $getusers = $this->connect()->prepare("SELECT * FROM users WHERE mail = ? AND password = ? || username = ? AND password = ?");
                $getusers->execute(array($usermail, $password, $usermail, $password));
                $usersexist = $getusers->rowCount();

                if ($usersexist == 1) {
                    $usersinfo = $getusers->fetch();
                    $_SESSION['accord_id_user'] = $usersinfo['id'];
    ...
    public function delete()
    {
        ...
        $delete->execute(array($_SESSION['accord_id_user']));
    ...
    public function update($mail, $username, $password, $phone)
    {
        ...
        if ($mail != '') {
            ...
            if (filter_var($mail, FILTER_VALIDATE_EMAIL)) {
                ...
                $updatemail->execute(array($mail, $_SESSION['accord_id_user']));
        ...
        if ($username != '') {
            ...
            if ($usernamelenght >= 3 && $usernamelenght <= 18) {
                ...
                $updateusername->execute(array($username, $_SESSION['accord_id_user']));
        ...
        if ($password != '') {
            ...
            $updatepassword->execute(array($password, $_SESSION['accord_id_user']));
        ...
        if ($phone != '') {
            if (preg_match("/^[0-9]{10}$/", $phone)) {
                ...
                $updatephone->execute(array($phone, $_SESSION['accord_id_user']));
    ...
    public function sendAvatar($urlavartar)
    {
        ...
        $sendavatar->execute(array($urlavartar, $_SESSION['accord_id_user']));
    }
}
